...
### 1.1a Shot manifest schema v2 — transition không thuộc shot

Frame được detector chấm là transition có thể nằm ngoài mọi shot để Begin/Middle/End không
ưu tiên fade/cut làm visual evidence. Không được bỏ các frame này một cách ngầm định. Mỗi
manifest phải ghi rõ phần bù giữa các shot:
...
Range phải tăng dần, không overlap, nằm trong video và khớp chính xác mọi gap giữa các shot;
sai accounting phải fail closed. Nếu `excluded_frame_fraction > 0.01`, validator phát warning
nhưng vẫn cho publish để người vận hành kiểm tra threshold quá nhạy. Keyframe planner không
được chọn frame thuộc `excluded_transition_ranges`.
...
- **24/08/2026** — Chốt giữ frame transition ngoài shot để Begin/Middle/End không ưu tiên
  fade/cut làm visual evidence. Shot manifest schema v2 bắt buộc ghi rõ từng range bị loại
